...
setwd("/Users/joshfelton/Desktop/PCA/Populus/populus_353")


#call VCF file
vcf.fn <- "Populus_A53_contig_LD_pruned_SNPs.recode.vcf"

#convert to GDS format for SNPRelate analysis
snpgdsVCF2GDS(vcf.fn, "test.gds", method="biallelic.only", ignore.chr.prefix = "CM")
...
#palette <- c("#800000","#f58231","#808000","#3cb44b","#4363d8","#dcbeff")

# black symbols instead of colors to standardize
color <- "black"
symbol.list <- c(20, 8, 9, 4, 3, 10, 2)
...
#plot PC 1 vs PC 2
pdf("PCA_unique_colors_PC1vsPC2.pdf",20,20)
par(mar=c(8.5,8.5,1.5,2), mgp=c(6,2,0))
col.list <- symbol.list

plot(tab$EV1, tab$EV2, col=color, cex=4.5, 
     pch=symbol.list[as.integer(tab$pop)], cex.lab=3.1, cex.axis=4, 
     xlab=paste0("eigenvector 1 (", rounded[1], "%)"), 
     ylab=paste0("eigenvector 2 (", rounded[2], "%)"))

legend("topright", legend=levels(tab$pop), cex=4, pch=symbol.list[1:nlevels(tab$pop)], 
       col=color)
dev.off()

#plot PC 2 vs PC 3
pdf("PCA_unique_colors_PC2vsPC3.pdf",20,20)
par(mar=c(6,6,1.5,2))
col.list <- palette
plot(tab$EV2,tab$EV3, col=col.list[as.integer(tab$pop)], cex=4, pch=16,cex.lab=3,cex.axis=2, xlab=paste0("eigenvector 2 (",rounded[2],"%)"), ylab=paste0("eigenvector 3 (",rounded[3],"%)"))
legend("bottomleft", legend=levels(tab$pop), cex=1.6,pch=16, col=col.list[1:nlevels(tab$pop)])
dev.off()

#plot PC 3 vs PC 4
pdf("PCA_unique_colors_PC3vsPC4.pdf",20,20)
par(mar=c(6,6,1.5,2))
col.list <- palette
plot(tab$EV3,tab$EV4, col=col.list[as.integer(tab$pop)], cex=4, pch=16,cex.lab=3,cex.axis=2, xlab=paste0("eigenvector 3 (",rounded[3],"%)"), ylab=paste0("eigenvector 4 (",rounded[4],"%)"))
